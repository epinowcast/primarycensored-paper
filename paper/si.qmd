...
This section details the derivation of analytical solutions for primary event censored distributions using the adjusted cumulative distribution function (CDF) approach described in the main text. This approach marginalises over primary event times to compute $F_{\text{cens}}(q) = \Pr(T_{\text{cens}} < q)$, providing both computational efficiency and statistical rigour.
