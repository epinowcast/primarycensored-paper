...
For any delay distribution with an available CDF, we can solve for the adjusted delay time CDF $F_{\text{adj}}$ by evaluating a single integral (see main text; equation ?). For distributions with particularly advantageous properties, we can derive fully analytical solutions that avoid numerical integration entirely.
