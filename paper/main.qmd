...
Although both the true delay $T$ and the censoring adjusted delay $T_{\text{cens}}$ are latent unobserved quantities, recasting the problem in terms of the censoring adjusted delay allows us to separate our analysis into two tractable problems by converting from a double interval censoring problem into a single interval censoring problem with an censoring adjusted delay distribution.

The first problem is relating the observed censoring delay $T_c$ to the unobserved censoring adjusted delay $T_{\text{cens}}$. Because the beginning time of the adjusted delay is fixed to $t_P$ by construction, we can replace the true delay CDF in @eq-cdfbasic above with the censoring adjusted delay distribution,

$$
\Pr(T_c = t_S - t_P \mid \text{observed}) = \frac{F_{\text{cens}}(t_S + w_S - t_P) - F_{\text{cens}}(t_S - t_P)}{F_{\text{cens}}(D - t_P)}.
$$ {#eq-cdfadj}

The second problem is efficiently calculating the cumulative distribution function (CDF) of the censoring adjusted delay $F_{\text{cens}}(q) = \Pr(T_{\text{cens}} < q)$ so that we can apply @eq-cdfadj to constructing the PMF for $T_c$ and in likelihood calculations for estimating $\theta$ (@eq-loglike). In the next section, we focus on this derivation.
...
In this case, our censoring adjusted delay time CDF integral simplifies to:
...
The marginalisation approach may face limitations in very complex multiple-step models where the computational benefits diminish relative to latent variable approaches. However, even in scenarios with nested integrals, the use of analytical solutions and framing problems as nested primarycensored problems (i.e., a primarycensoring problem within a primarycensoring problem) may maintain computational advantages.
