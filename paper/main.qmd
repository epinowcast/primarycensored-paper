...
When $r = 0$, the exponentially tilted distribution @eq-condP reduces to the uniform distribution:

$$f_P(p) = \frac{1}{w_P} \mathbb{1}_{[t_P, t_P + w_P]}(t)$$
...
$$F_{\text{cens}}(q) = F_T(q-w_P) + \frac{1}{w_P} \int_0^{w_P} f_T(q-u) u \, dz.$$

This expression can be further simplified by another variable substitution setting $z = q - u$,

$$
\begin{aligned}
F_{\text{cens}}(q) &= F_T(q-w_P) + \frac{1}{w_P} \int_{q-w_P}^{q} f_T(z) (q-z) \, dz \\
 &= F_T(q-w_P) + \frac{q}{w_P} \left (F_T(q) - F_T(q-w_P) \right) - \int_{q-w_P}^{q} z f_T(z)  \, dz 
\end{aligned} 
$$ {#eq-analytic}

For parametric distributions the CDF of the delay $T$ is analytically accessible, therefore, solving @eq-analytic for any given value of $q$ depends on resolving the final integral term.

#### The role of partial expectation in analytical solutions

Here, we encounter what's known as a partial expectation of the delay distribution:

$$\int_{q-w_P}^{q} z f_T(z) \, dz$$ {#eq-partialint}

The partial expectation @eq-partialint calculates the contribution to the mean of $T$ from values within the interval [q-w_P, q]. Similarly to how many parametric distributions have an associated parametric form for the distribution mean, the partial expectation @eq-partialint also has parametric forms dependent on the distribution parameters and the interval location.

This relationship enables us to develop analytical solutions that avoid numerical integration for distributions with these closed-form partial expectations.
