...
The adjusted delay time CDF, $F_{\text{adj}}(q)$, represents the probability that the sum of the time between the beginning of the primary window and the primary event time $P - t_P$ and the delay time $T$ is less than or equal to $q$, given that the primary event occurred within its censoring window. 

$$F_{\text{adj}}(q) = P(T + (P - t_P) \leq q \mid P_u \in [t_P, t_P + w_P])$$

Because the delay time $T$ is independent of $P_u$, we can conveniently write

$$F_{\text{adj}}(q) = P(T + \tilde{P} \leq q),$$

Where $\tilde{P} = (P - t_P) | P_u \in [t_P, t_P + w_P])$. Using a standard identity for the cumulative distribution of the sum of random variables we reduce the calculation of $F_{\text{adj}}$$ to,

$$F_{\text{adj}}(q) = \mathbb{E}_{\tilde{P}}[P(T \leq (q - \tilde{P})]$$

For continuous random variables, expectations are calculated using integrals. This gives us:

$$F_{\text{adj}}(q) = \int_{0}^{w_P} P(T \leq q - p ) \cdot f_{\tilde{P}}(p) \, dp  = \int_{0}^{w_P} F_T(q - p) \cdot f_P(t_P + p) \, dp.$$
