...
The primary event censored CDF, $F_{\text{cens}}(q)$, represents the probability that the delay is less than or equal to $q$, given that the primary event occurred within its censoring window:

$$F_{\text{cens}}(q) = \int_{t_P}^{t_P + w_P} F_T(q - (p - t_P)) \cdot f_P(p) \, dp$$

where $F_T$ is the CDF of the true delay distribution, $f_P$ is the PDF of the primary event within its censoring window, and the integration is over all possible primary event times within the window.

For computational simplicity, we can shift the integration variable to measure time from the start of the primary event window:

$$F_{\text{cens}}(q) = \int_{0}^{w_P} F_T(q - p) \cdot f_P(t_P + p) \, dp$$

We can now use numerical integration to evaluate this integral for any delay distribution and so we have a general solution for the primary censored CDF. In the next section, we will explore analytical solutions for common parametric distributions.

### Analytical solutions

#### General approach

While this integral can be evaluated numerically for any delay distribution, we can derive more efficient solutions by considering the survival function approach. Let $S_+$ be the time from the end of the primary censoring interval to the secondary event, and $C_P = w_P - p$ be the time from the primary event to the end of its censoring window. The survival function of $S_+$ is:

$$Q_{S_+}(t) = P(S_+ > t) = P(T > C_P + t) = \mathbb{E}_{C_P}[Q_T(t + C_P)]$$

where $Q_T(t) = 1 - F_T(t)$ is the survival function of the true delay distribution.

Using integration by parts, we can rewrite this as:

$$Q_{S_+}(t) = Q_T(t + w_P) + \int_0^{w_P} f_T(t+p) \cdot F_{C_P}(p) \, dp$$

where $f_T$ is the PDF of the true delay distribution and $F_{C_P}$ is the CDF of the time from the primary event to the end of its censoring window.

This formulation has two key advantages. First, for many common distributions, such as gamma, lognormal, and Weibull, the integral can be analytically solved when the primary event is uniformly distributed within its window. Second, even when analytical solutions are not available, this approach is computationally more efficient for numerical integration than direct evaluation of the primary censored CDF.

The relationship between the survival function $Q_{S_+}$ and the primary censored CDF $F_{\text{cens}}$ is:

$$F_{\text{cens}}(q) = 1 - Q_{S_+}(q - w_P)$$

This approach allows us to efficiently compute the primary censored CDF for a wide range of delay distributions. 
