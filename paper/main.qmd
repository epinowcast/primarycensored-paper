...
With non-informative censoring (where the observation process is independent of the actual event times), we model the distribution of the primary event within its window as:
...
This will reduce the @eq-Fadj to a simple form whenever $f_P$ is modelled as a simple distribtuion.
...
$$F_{\text{adj}}(q) = F_T(q) + \frac{1}{w_P} \left[ \int_q^{q+w_P} f_T(z) (z-q) \, dz \right]$$
...
Our analytical and numerical solutions accurately reproduced the probability mass functions (PMFs) generated by Monte Carlo simulations across all censoring and truncation scenarios. For distributions with analytical solutions (gamma and lognormal), the analytically derived PMFs and numerically integrated PMFs were effectively identical, confirming the correctness of our theoretical derivations. 

The numerical solutions maintained high accuracy even for the Burr distribution, which lacks an analytical solution, validating our quadrature approach for arbitrary distributions. This consistency persisted across all sample sizes, with expected increases in Monte Carlo variability at smaller sample sizes.

Run time comparisons demonstrated substantial efficiency gains with our methods. The analytical solutions were approximately 100-1000 times faster than full Monte Carlo simulation for datasets with 10,000 observations. Even the numerical integration approach was 10-100 times faster than simulation-based methods. This performance advantage increased with larger datasets, demonstrating the scalability benefits of our approach for large-scale surveillance data.

### Parameter recovery results

Our primary censored distribution approach accurately recovered the true delay distribution parameters across all simulation scenarios. For gamma distributions, both shape and scale parameters were recovered with minimal bias, even under severe truncation conditions. Similar accuracy was observed for lognormal and Burr distributions, with marginally wider uncertainty intervals for the Burr distribution reflecting its more complex parametrisation.

In contrast, naive methods that ignored censoring and truncation consistently underestimated the mean of the delay distributions, with bias increasing proportionally to censoring window width and the severity of truncation. This bias was particularly pronounced in the severe truncation scenario, where the naive approach underestimated the mean delay by up to 40%.

Both Bayesian and maximum likelihood implementations demonstrated similar parameter recovery accuracy. The Bayesian models showed good convergence across all scenarios with R-hat values consistently below 1.01 and no divergent transitions in the MCMC sampling. The Stan implementation also demonstrated computational efficiency, producing more effective samples per second than comparable latent variable approaches.

### Case study results

In the EVD case study, our method estimated symptom onset to sample test delay distributions that closely aligned with those from the latent variable approach of Ward et al. across all observation periods. Both methods produced similar estimates of the gamma shape and scale parameters, confirming that our marginalisation approach maintains the statistical integrity of the current best practice.

However, our method demonstrated significant computational advantages, achieving approximately 20-30 times more effective samples per second than the latent variable approach. This efficiency gain was particularly evident in the later observation periods (120-180 and 180-240 days) where the dataset size was larger. The naive approach produced substantially different parameter estimates, particularly in real-time analyses of recent data where truncation effects were strongest.

The differences between real-time and retrospective analyses highlighted the importance of accounting for right truncation in delay distribution estimation. Both our method and the Ward et al. approach showed consistent parameter estimates between real-time and retrospective analyses when properly accounting for truncation, while the naive approach showed significant differences.
...
The main limitation of our approach is that we separate primary and secondary censoring, treating them as independent problems rather than using a joint interval approach. While this simplifies the mathematical formulation and implementation considerably, it may be less optimal in scenarios where the joint distribution structure is complex. Additionally, our approach currently handles only relatively simple forms of primary event time distributions within censoring windows (uniform and exponentially tilted), whereas real-world reporting patterns may involve more complex distributions.

The Ward et al. latent parameter approach offers simplicity and flexibility for individual-level customisation of observation models, but at substantial computational cost for large datasets. It also requires a small approximation in the denominator term when handling truncation. Our approach addresses these limitations but sacrifices some of the flexibility for individual-level customisation.
