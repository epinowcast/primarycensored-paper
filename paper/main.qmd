...
To recover unbiased estimates of the delay distribution parameters from observed data, we begin by considering the PMF of the censored delay time $T_c$ in the simplest case of only secondary event censoring with right truncation, and then extend to include primary event censoring. This is equivalent to having a primary censoring window of zero width, $w_P = 0$.

When only the secondary event is interval-censored, our goal is to calculate the probability that a delay falls into a specific observed bin. For a secondary event observed in the interval $[t_S, t_S + w_S]$ given that the primary event occurred at a precisely known time $P_u = t_P$, and conditioned on the secondary event occurring before the maximum observable time $D$, the censored delay PMF is:

$$P(T_c = t_S + w_S - t_P \mid \text{observed}, P_u = t_p) = \frac{P(S \in [t_S, t_S + w_S] \mid P_u = t_P)}{P(S < D \mid P_u = t_P)}$$

Because the primary time is known, this can be written in terms of the CDF of the true delay distribution $F_T$:

$$P(T_c = t_S + w_S - t_P ) \mid \text{observed}, P_u = t_p) = \frac{F_T(t_S + w_S - t_P) - F_T(t_S - t_P)}{F_T(D - t_P)}$$
...
With primary event censoring, the primary event is known only to have occurred within the interval $[t_P, t_P + w_P]$. Therefore, the time after $t_P$ until the secondary event will be longer than the true delay since it includes the time between the start of the primary window and the primary event time. We call this delay time after the beginning of the primary window $T_{\text{adj}}$, which is the adjusted delay time that accounts for the primary event censoring. We introduce $F_{\text{adj}}(q)$ as the cumulative distribution function (CDF) of the adjusted delay distribution:

$$F_{\text{adj}}(q) = P(T_{\text{adj}} \leq q \mid P_u \in [t_P, t_P + w_P])$$

Once we have $F_{\text{adj}}$, we can extend our formula for secondary event censoring by substituting $F_{\text{adj}}$ for $F_T$. This gives a general form for the censored delay time PMF:

$$P(T_c = t_S + w_S - (t_P + w_P) \mid \text{observed}) = \frac{F_{\text{cens}}(t_S + w_S - t_P) - F_{\text{cens}}(t_S - t_P)}{F_{\text{cens}}(D - t_P)}.$$

which is equivalent to,

$$P(T_c = t_S + w_S - (t_P + w_P) \mid \text{observed}) = \frac{P(S \in [t_S, t_S + w_S] \mid P_u \in [t_P, t_P + w_P])}{P(S < D \mid P_u \in [t_P, t_P + w_P])}$$

This approach breaks the problem of double censoring and right truncation into separable components: first, we calculate the CDF for the adjusted delay time ($F_{\text{adj}}$), second, we apply the adjusted delay time CDF to the secondary censored window. In the next section, we focus on deriving the adjusted delay time CDF $F_{\text{adj}}$.
...
The adjusted delay time CDF, $F_{\text{adj}}(q)$, represents the probability that the sum of the time between the beginning of the primary window and the primary event time $P_u - t_P$ and the delay time $T$ is less than or equal to $q$, given that the primary event occurred within its censoring window. 

$$F_{\text{adj}}(q) = P(T + (P_u - t_P) \leq q \mid P_u \in [t_P, t_P + w_P])$$

Because the delay time $T$ is independent of $P_u$, we directly use the conditional primary time defined above,

$$F_{\text{adj}}(q) = P(T + P - t_P \leq q),$$

Using a standard identity for the cumulative distribution of the sum of random variables we reduce the calculation of $F_{\text{adj}}$ to,

$$F_{\text{adj}}(q) = \mathbb{E}_{P}[P(T \leq (q + t_P - P)]$$
...
where $f_P$ is the conditional PDF of the primary event within its censoring window.
...
$$F_{\text{adj}}(q) = \int_{0}^{w_P} F_T(q - u) \cdot f_P(t_P + u) \, du.$$

This formulation gives us a general solution to the adjusted delay time CDF as a single variable integral problem. In turn, solving for $F_{\text{adj}}$ solves for the censored delay PMF in an exact manner that properly accounts for the uncertainty in the primary event time. Because $F_{\text{adj}}$ is a single variable integral problem, we can use numerical quadrature to evaluate this integral efficiently for any delay distribution with an analytic representation of its distribution function, or derive analytical solutions for specific parametric distributions, as we will explore in the next section. See the SI for an alternative treatment based on the survival functions and connections to the approaches of Park et al. [@Park2024.01.12.24301247], Reich et al. [@Reich2009-aa], and Cori et al. [@Cori2013].
