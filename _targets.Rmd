...
tar_target(
  primarycensored_fits,
  {
    ...
    # Real implementation would use primarycensored package
    # with both analytical (gamma, lognormal) and numerical (Burr) solutions
    
    message(paste("Fitting primarycensored model for:", simulated_data$scenario_id[1]))
...
tar_target(
  naive_fits,
  {
    ...
    message(paste("Fitting naive model for:", simulated_data$scenario_id[1]))
...
tar_target(
  ward_fits,
  {
    ...
    message(paste("Fitting Ward et al. model for:", simulated_data$scenario_id[1]))
...
tar_target(
  all_model_fits,
  {
    # Combine results from all fitting methods
    dplyr::bind_rows(
      primarycensored_fits,
      naive_fits,
      ward_fits
...
tar_target(
  ...
  {
    # Extract R-hat, divergences, ESS from Bayesian fits
    all_model_fits |>
      dplyr::filter(method %in% c("primarycensored", "ward")) |>
      dplyr::select(scenario_id, method, convergence) |>
      tidyr::unnest_wider(convergence)
  }
)
```
