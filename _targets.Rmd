...
tar_target(
  ebola_delay_data,
  {
    # Transform dates to delays for this window/analysis combination
    delay_data <- transform_ebola_to_delays(ebola_case_study_data)
    
    # Add window metadata to transformed data
    delay_data |>
      dplyr::mutate(
        ...
        start_day = ebola_case_study_data$start_day,
        end_day = ebola_case_study_data$end_day
      )
  },
...
```{targets ebola_data_summary}
tar_target(
  ebola_data_summary,
  {
    # Combine all delay data branches and summarise
    combined_delay_data <- dplyr::bind_rows(ebola_delay_data)
    summarise_ebola_windows(combined_delay_data)
  }
)
```
...
tarchetypes::tar_group_by(
  ...
  create_fitting_grid(
    ...
    ebola_delay_data = dplyr::bind_rows(ebola_delay_data),
    scenarios = scenarios,
