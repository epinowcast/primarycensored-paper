...
Both primary and secondary events have censoring windows ranging from 1-7 days.
...
tar_target(
  simulated_data,
  {
    ...
    # Generate primary event times using exponential growth
    n_obs <- params$n
    max_time <- 30  # Maximum time for exponential growth window
    prim_times <- rexpgrowth(n_obs, min = 0, max = max_time, r = growth_rate)
    
    # Generate delays using rprimarycensored with proper parameter passing
    # Create distribution arguments
    dist_args <- list(n = n_obs)
    if (!is.na(params$param1)) {
      param_names <- names(formals(get(paste0("r", params$dist_family))))
      dist_args[[param_names[2]]] <- params$param1
      if (!is.na(params$param2)) {
        dist_args[[param_names[3]]] <- params$param2
      }
    }
    
    delays <- rprimarycensored(
      n = n_obs,
      rdist = function(n) do.call(get(paste0("r", params$dist_family)), dist_args),
      rprimary = function(n, pwindow) rexpgrowth(n, min = 0, max = pwindow, r = growth_rate),
      pwindow = params$primary_width,
      swindow = params$secondary_width,
      D = params$relative_obs_time
    )
    ...
    data.frame(
      ...
      dplyr::filter(sec_time <= prim_time + params$relative_obs_time)  # Apply truncation
  },
